...
Regular users are allowed to perform 'create', 'read' and 'list' operations only in the tenants they have access to.
They can also perform 'update' and 'delete' operations, but only on the objects they created.

Admin users could perform any operation in any tenant and on any object.

Whenever a user wants to perform an operation in a tenant, which is not his default,
the user must specify 'tenant_id' attribute for the object. Since admin user does not have a default tenant,
he must specify 'tenant_id' for every operation he performs.
...
.. sourcecode:: http
    ...
    GET /v0.1/clusters?name=foo1&some_attr=some_value
...
.. sourcecode:: http
    ...
    GET /v0.1/clusters.json?fields=id&fields=name&fields=status
...
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
| node_type                          | string (enum)                              | A type of the node                                  |
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
| tenant_id                          | string                                     | An id of the OpenStack's tenant                     |
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
...
In fact, a node is a virtual machine managed by OpenStack Nova.
...
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
| node_template_id                   | string                                     | Node Template name used to create this node         |
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
...
+====================================+============================================+=====================================================+
| GET                                | /v0.1/node-templates                       | Lists summary for all Node Templates in EHO that are|
|                                    |                                            | accessible to the tenant who submits the request    |
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
| POST                               | /v0.1/node-templates                       | Creates a new Node Template                         |
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
| GET                                | /v0.1/node-templates/{node_template_id}    | Lists information for the specified Node Template   |
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
| PUT                                | /v0.1/node-templates/{node_template_id}    | Updates a specific Node Template identified by      |
|                                    |                                            | node_template_id                                    |
|                                    |                                            | (only unused Node Templates could be modified)      |
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
| DELETE                             | /v0.1/node-templates/{node_template_id}    | Destroyed a specified Node Template                 |
...
.. http:get:: /v0.1/node-templates

Normal Response Code: 200 (OK)

Errors: none

This operations returns the list of all Node Templates associated with your tenant.
If you have an admin role, then this request returns all the Node Templates of all tenants.
...
Users with an admin role can create Node Templates on behalf of other tenants by specifying a tenant_id parameter
different than their own. The tenant_id attribute is only required if the caller has an admin role and wants to
create a Node Template for another tenant.
...
**Example**:
    **request**
    ...
    .. sourcecode:: http
        ...
        POST http://eho/v0.1/node-templates
...
.. http:get:: /v0.1/node-templates/{node_template_id}
...
If the user is not an admin, and the Node Template object doesn't belong to user's tenant,
user would receive a 403 (Forbidden) error.
...
**Example**:
    **request**
    ...
    .. sourcecode:: http
        ...
        GET http://eho/v0.1/node-templates/3421
...
.. http:put:: /v0.1/node-templates/{node_template_id}
...
**Example**:
    **request**
    ...
    .. sourcecode:: http
        ...
        PUT http://eho/v0.1/node-templates/3413
...
.. http:delete:: /v0.1/node-templates/{node_template_id}
...
Any and all data is immediately purged and is not recoverable. If the user is not an admin, and the Node Template
object doesn't belong to user's tenant, user would receive a 403 (Forbidden) error. Only Node Templates that are
not in use could be removed. So, if any cluster used any Node Template than such Node Template could not be modified
or removed.
...
**Example**:
    **request**
    ...
    .. sourcecode:: http
        ...
        DELETE http://eho/v0.1/node-templates/3421
...
| Verb                               | URI                                        | Description                                         |
+====================================+============================================+=====================================================+
| GET                                | /v0.1/clusters                             | Lists summary for all clusters in EHO that are      |
|                                    |                                            | accessible to the tenant who submits the request    |
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
| POST                               | /v0.1/clusters                             | Creates a new cluster                               |
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
| GET                                | /v0.1/clusters/{cluster_id}                | Lists detailed information for the specified cluster|
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
| PUT                                | /v0.1/clusters/{cluster_id}                | Updates a specific cluster identified by cluster_id |
+------------------------------------+--------------------------------------------+-----------------------------------------------------+
| DELETE                             | /v0.1/clusters/{cluster_id}                | Destroys a specified cluster                        |
+------------------------------------+--------------------------------------------+-----------------------------------------------------+

3.2.2 List all clusters
-----------------------

.. http:get:: /v0.1/clusters

Normal Response Code: 200 (OK)

Errors: none

This operations returns the list of all clusters associated with your tenant.
If you have an admin role, then this request returns all the clusters of all tenants.
...
Users with an admin role can create clusters on behalf of other tenants by specifying a tenant_id parameter
different than their own. The tenant_id attribute is only required if the caller has an admin role and wants
to create a cluster for another tenant.
...
.. http:get:: /v0.1/clusters/{cluster-id}
...
This operation returns a cluster object identified by cluster_id. If the user is not an admin,
and the cluster object doesn't belong to user's tenant, user would receive a 403 (Forbidden) error.
...
**Example**:
    **request**
    ...
    .. sourcecode:: http
        ...
        GET http://eho/v0.1/clusters/1234
...
.. http:put:: /v0.1/clusters/{cluster-id}
...
**Example**:
    **request**
    ...
    .. sourcecode:: http
        ...
        PUT http://eho/v0.1/clusters/1234
...
.. http:delete:: /v0.1/clusters/{cluster-id}
...
Any and all data is immediately purged and is not recoverable. If the user is not an admin, and the cluster object
doesn't belong to user's tenant, user would receive a 403 (Forbidden) error.
...
**Example**:
    **request**
    ...
    .. sourcecode:: http
        ...
        DELETE http://eho/v0.1/clusters/1234
